...
import { PROUDCT_SORT_FIELDS, SORT_ORDER_ASC, SORT_ORDER_DESC } from "@/lib/constant";

type ProductSearchSortProps = {
    search: string;
    setSearch: React.Dispatch<React.SetStateAction<string>>;
    sortField: keyof Product;
    sortOrder: typeof SORT_ORDER_ASC | typeof SORT_ORDER_DESC;
    setSortField: React.Dispatch<React.SetStateAction<keyof Product>>;
...
export default function ProductSearchSort({
    ...
    const toggleSortOrder = (field: keyof Product) => {
        if (sortField === field) {
    ...
    return (
        <div className="bg-white rounded-lg p-6 shadow-md flex flex-wrap gap-4 items-center border border-gray-200">
            ...
            <div className="flex items-center gap-4">
                ...
                {PROUDCT_SORT_FIELDS.map((field) => (
                    <button
                        key={field}
                        onClick={() => toggleSortOrder(field as keyof Product)}
