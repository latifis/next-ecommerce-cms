...
import { BRAND_SORT_FIELDS, SORT_ORDER_ASC, SORT_ORDER_DESC } from "@/lib/constant";

type BrandSearchSortProps = {
    search: string;
    setSearch: React.Dispatch<React.SetStateAction<string>>;
    sortField: keyof Brand;
    sortOrder: typeof SORT_ORDER_ASC | typeof SORT_ORDER_DESC;
    setSortField: React.Dispatch<React.SetStateAction<keyof Brand>>;
    setSortOrder: React.Dispatch<React.SetStateAction<typeof SORT_ORDER_ASC | typeof SORT_ORDER_DESC>>;
    setPageSize: React.Dispatch<React.SetStateAction<number>>;
};

export default function BrandSearchSort({
    search,
    setSearch,
    sortField,
    sortOrder,
    ...
    setSortOrder,
    setPageSize,
}: BrandSearchSortProps) {
    const toggleSortOrder = (field: keyof Brand) => {
        if (sortField === field) {
    ...
    return (
        <div className="bg-white rounded-lg p-6 shadow-md flex flex-wrap gap-4 items-center border border-gray-200">
            ...
            <div className="flex items-center gap-4">
                ...
                {BRAND_SORT_FIELDS.map((field) => (
                    <button
                        key={field}
                        onClick={() => toggleSortOrder(field as keyof Brand)}
